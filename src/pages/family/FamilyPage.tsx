...
const FamilyPage = () => {
    ...
    const { families, createFamily, isLoading } = useFamily();
    ...
    const openCreateModal = () => {
        if (limits && limits.tier !== 'family' && limits.tier !== 'team') {
            toaster.create({
                title: "Family Feature",
                description: "Family groups are available on Family and Teams plans.",
                type: "info",
                ...
                <VStack align="stretch" spaceY={8}>
                    ...
                    {families.length === 0 && !isLoading ? (
                        <VStack py={20} gap={6} textAlign="center" border="1px dashed" borderColor="border.subtle" rounded="2xl" bg="bg.subtle/30">
                            ...
                            <VStack gap={2}>
                                <Heading size="lg">No Family Groups Yet</Heading>
                                <Text fontSize="md" color="fg.muted" maxW="md">
                                    Create a family group to share passwords and 2FA codes with your loved ones.
                                </Text>
                            </VStack>
                        ...
                        <SimpleGrid columns={{ base: 1, md: 2, lg: 3 }} gap={6}>
                            {families.map((family, index) => (
                                <Link to={`/family/${family._id}`} key={index} style={{ textDecoration: 'none' }}>
                                    <SpotlightCard
                                        h="full"
                                        rounded="2xl"
                                        border="1px solid"
            ...
            <DialogRoot open={isCreateOpen} onOpenChange={(e) => setIsCreateOpen(e.open)}>
                <DialogContent>
                    ...
                    <DialogBody>
                        <VStack gap={4}>
                            <Text fontSize="sm" color="fg.muted">
                                Name your family group (e.g., "The Johnsons").
                            </Text>
                            <Input
                    ...
                    <DialogFooter>
                        ...
                        <AppButton variant="primary" onClick={handleCreateFamily}>
                            Create Group
                        </AppButton>
                    </DialogFooter>
                    <DialogCloseTrigger />
